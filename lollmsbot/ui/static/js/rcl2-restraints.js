...
class RestraintMatrix {
    ...
    render(panel, data) {
        ...
        const cardsHTML = categories.map(([category, dimensions]) => {
            const controlsHTML = dimensions.map(dim => {
                ...
                // Handle rich object or simple value
                let value = this.restraints[key];
                if (typeof value === 'object' && value !== null && 'ui_value' in value) {
                    value = value.ui_value;
                }
                value = value || 0;
                ...
                const displayValue = isNaN(val) ? '0.00' : val.toFixed(2);
                ...
                return `
                    <div class="restraint-control">
                        ...
                        <div class="restraint-hint">
                            ...
                            ${hardLimit !== null ? ` <strong>Hard limit: ${Number(hardLimit).toFixed(2)} (can reduce below this)</strong>` : ''}
                        </div>
                    </div>
                `;
            }).join('');
    ...
    handleSliderChange(dimension, value) {
        ...
        // Track change
        let originalValue = this.restraints[key];
        if (typeof originalValue === 'object' && originalValue !== null && 'ui_value' in originalValue) {
            originalValue = originalValue.ui_value;
        }
