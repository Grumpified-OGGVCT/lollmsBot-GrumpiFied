...
class MultiProviderRouter:
    ...
    async def chat(
        ...
        **kwargs
        ...
        Strategy:
        1. Try OpenRouter free tier first (cycle through 3 keys)
        2. If all OpenRouter keys exhausted, notify and fallback to Ollama
        3. For Ollama-specific models, skip OpenRouter
        
        Args:
            messages: Chat messages
            model: Optional model name (if Ollama-specific, skips OpenRouter)
        ...
        for provider in self.providers:
            ...
            try:
                logger.info(f"Trying provider: {provider.config.name}")
                ...
                # Notify user when OpenRouter exhausted
                if isinstance(provider, OpenRouterProvider):
                    logger.info("OpenRouter free tier exhausted, falling back to Ollama")
