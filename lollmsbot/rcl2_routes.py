...
async def verify_ws_token(websocket: WebSocket, token: Optional[str] = Query(None)):
    ...
    Allows localhost connections without tokens, matching HTTP require_auth behavior.
    Only requires token for non-local connections when RCL2_WS_TOKEN is set.
    """
    # Get client host
    client_host = websocket.client.host if websocket.client else "unknown"
    
    # Always allow localhost connections (security handled at network level)
    if client_host in ("127.0.0.1", "::1", "localhost"):
        return True
    
    # For non-local connections, check token if configured
...
class RestraintUpdateRequest(BaseModel):
    ...
    def validate_auth_key(cls, v):
        ...
        if v and not re.match(r'^[0-9a-fA-F]+$', str(v)):
...
class DeliberationRequest(BaseModel):
    ...
    stakes: str = Field(default="medium", description="Stakes level", pattern="^(low|medium|high|critical)$")
    
    @field_validator('action_id', 'action_type', mode='before')
    @classmethod
    def validate_alphanumeric(cls, v, info):
...
class DebtRepaymentRequest(BaseModel):
    ...
    def validate_decision_id(cls, v):
        ...
        if v and not re.match(r'^[a-zA-Z0-9_-]+$', str(v)):
            ...
            "deliberations": [
                {
                    ...
                    "unanimous": delib.unanimous,
                    "timestamp": delib.action.timestamp,
                }
                for delib in recent
            ],
        }
...
async def get_decisions(
    ...
    try:
        ...
        # Get all decisions
        decisions = manager.get_decision_history(limit=1000)
